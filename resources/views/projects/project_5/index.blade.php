<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Project_5</title>
    <link rel="shortcut icon" href="favicon.ico" type="image/x-icon">
    <link rel="stylesheet" href="{{ asset('assets/project_5/css/style.css') }}">
    <link href="https://fonts.googleapis.com/css?family=Montserrat:regular,500,600,700,800&display=swap" rel="stylesheet" />
</head>
<body>
    <div class="wrapper">
        <header class="header">
            <div class="header__container _container">
                <a href="#" class="header__logo">Relvise</a>
                <nav class="header__menu menu">
                    <ul class="menu__list">
                        <li class="menu__item">
                            <a href="#" class="menu__link">
                                Home
                            </a>
                        </li>
                        <li class="menu__item">
                            <a href="#" class="menu__link">
                                Product
                            </a>
                        </li>
                        <li class="menu__item">
                            <a href="#" class="menu__link">
                                Pricing
                            </a>
                        </li>
                        <li class="menu__item">
                            <a href="#" class="menu__link">
                                Contact
                            </a>
                        </li>
                    </ul>
                </nav>
            </div>
        </header>
        <main class="page">
            {{-- promo --}}
            <div class="page__main-block main-block">
                <div class="main-block__container _container">
                    <div class="main-block__body">
                        <h1 class="main-block__title">
                            Finance and Consultancy Solution
                        </h1>
                        <div class="main-block__text">
                            We know how large objects will act, but things on a small scale.
                        </div>
                        <div class="main-block__buttons">
                            <a href="#">
                                <button class="main-block__button main-block__button_orange">
                                    Get Quote Now
                                </button>
                            </a>
                            <a href="#">
                                <button class="main-block__button main-block__button_border">
                                    Learn More
                                </button>
                            </a>
                        </div>
                    </div>
                </div>
                <div class="main-block__image _ibg">
                    <img src="img/cover.jpg" alt="cover">
                </div>
            </div>
            {{-- services --}}
            <section class="page__services services">
                <div class="services__container _container">
                    <div class="services__body">
                        <div class="services__row">
                            <div class="services__column">
                                <div class="services__item item-service">
                                    <div class="item-service__icon">
                                        <img src="img/project_5/1.svg" alt="Environmental Consulting">
                                    </div>
                                    <div class="item-service__title">Environmental Consulting</div>
                                    <div class="item-service__text">We focus on ergonomics and meeting you where you work. </div>
                                </div>
                            </div>
                            <div class="services__column">
                                <div class="services__item item-service">
                                    <div class="item-service__icon">
                                        <img src="img/project_5/2.svg" alt="Finance and consultancy">
                                    </div>
                                    <div class="item-service__title">Finance and consultancy</div>
                                    <div class="item-service__text">Just type what's on your mind and we'll get you there.</div>
                                </div>
                            </div>
                            <div class="services__column">
                                <div class="services__item item-service item-service_green">
                                    <div class="item-service__icon">
                                        <img src="img/project_5/3.svg" alt="Financial Services Consulting">
                                    </div>
                                    <div class="item-service__title">Financial Services Consulting</div>
                                    <div class="item-service__text">the quick fox jumps over the lazy dog</div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
            {{-- experience --}}
            <section class="page-experience experience">
                <div class="experience__container _container">
                    <h2 class="experience__title title-block">
                        Designing Better Experience
                    </h2>
                    <div class="experience__text subtitle-block">
                        Problems trying to resolve the conflict between the two major realms
                        of Classical physics: Newtonian mechanics
                    </div>
                    <div class="experience__body">
                        <div class="experience__row">
                            <div class="experience__column">
                                <div class="item-card">
                                    <div class="item-card__img">
                                        <img src="img/project_5/4.svg" alt="Designing Better Experience">
                                    </div>
                                    <div class="item-card__number">973 +</div>
                                    <div class="item-card__title">Consumer Products</div>
                                </div>
                            </div>
                            <div class="experience__column">
                                <div class="item-card">
                                    <div class="item-card__img">
                                        <img src="img/project_5/5.svg" alt="Financial Services">
                                    </div>
                                    <div class="item-card__number">181 +</div>
                                    <div class="item-card__title">Financial Services</div>
                                </div>
                            </div>
                            <div class="experience__column">
                                <div class="item-card">
                                    <div class="item-card__img">
                                        <img src="img/project_5/6.svg" alt="Environmental">
                                    </div>
                                    <div class="item-card__number">98%</div>
                                    <div class="item-card__title">Environmental</div>
                                </div>
                            </div>
                            <div class="experience__column">
                                <div class="item-card">
                                    <div class="item-card__img">
                                        <img src="img/project_5/1.svg" alt="Business & Finance">
                                    </div>
                                    <div class="item-card__number">746 +</div>
                                    <div class="item-card__title">Business & Finance</div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
            {{-- practice --}}
            <section class="page-practice practice">
                <div class="practice__container _container">
                    <h2 class="title-block practice__title">
                        Practice Advice
                    </h2>
                    <div class="subtitle-block practice__text">
                        Problems trying to resolve the conflict between
                        the two major realms of Classical physics: Newtonian mechanics
                    </div>
                    <div class="practice__row">
                        <div class="practice__column">
                            <div class="practice-card">
                                <a href="#">
                                    <div class="practice-card__header">A single source of truth</div>
                                </a>
                                <div class="practice-card__text">
                                    Newton thought that light was made up of particles, but then it was discovered
                                </div>
                                <a href="#">
                                    <div class="practice-card__img">
                                        <img src="img/project_5/7.jpg" width="230px" alt="A single source of truth">
                                    </div>
                                </a>
                            </div>
                        </div>
                        <div class="practice__column">
                            <div class="practice-card">
                                <a href="#">
                                    <div class="practice-card__header">Fastest way to organize</div>
                                </a>
                                <div class="practice-card__text">
                                    “Quantum mechanics” is the description of the behaviour of matter
                                </div>
                                <a href="#">
                                    <div class="practice-card__img">
                                        <img src="img/project_5/8.jpg" width="230px" alt="Fastest way to organize">
                                    </div>
                                </a>
                            </div>
                        </div>
                        <div class="practice__column">
                            <div class="practice-card">
                                <a href="#">
                                    <div class="practice-card__header">Fastest way to take action</div>
                                </a>
                                <div class="practice-card__text">
                                    They describe a universe consisting of bodies moving
                                </div>
                                <a href="#">
                                    <div class="practice-card__img">
                                        <img src="img/project_5/9.jpg" width="230px" alt="Fastest way to take action">
                                    </div>
                                </a>
                            </div>
                        </div>
                        <div class="practice__column">
                            <div class="practice-card">
                                <a href="#">
                                    <div class="practice-card__header">Work better together</div>
                                </a>
                                <div class="practice-card__text">
                                    Newton thought that light was made up of particles, but then it was discovered
                                </div>
                                <a href="#">
                                    <div class="practice-card__img">
                                        <img src="img/project_5/10.jpg" alt="Work better together">
                                    </div>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
            {{-- about --}}
            <section class="page-about about">
                <div class="">
                    <h2 class="title-block ">
                        Who We Are
                    </h2>
                    <div class="subtitle-block">
                        Problems trying to resolve the conflict between the two major realms
                        of Classical physics: Newtonian mechanics
                    </div>
                </div>
                <div class="about__row _container">
                    <div class="about__column">
                        <div class="about__img">
                            <iframe width="560" height="315" src="https://www.youtube.com/embed/zs1r8yafTE8" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                        </div>
                    </div>
                    <div class="about__column about__column_second">
                        <div class="about__title">
                            Most trusted in<br>our field
                        </div>
                        <div class="about__text">
                            Most calendars are designed for teams. Slate is designed for freelancers who want a simple way to plan their schedule.
                        </div>
                        <div class="about__list about-list">
                            <div class="about-list__row">
                                <div class="about-list__img">
                                    <img src="img/project_5/11.svg" alt="Most trusted">
                                </div>
                                <div class="about-list__desc">
                                    <h6>the quick fox jumps over the lazy dog</h6>
                                    <p>Things on a very small scale ...</p>
                                </div>
                            </div>
                            <div class="about-list__row">
                                <div class="about-list__img">
                                    <img src="img/project_5/12.svg" alt="Most trusted">
                                </div>
                                <div class="about-list__desc">
                                    <h6>the quick fox jumps over the lazy dog</h6>
                                    <p>Things on a very small scale ...</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
            {{-- advs --}}
            <div class="advs">
                <div class="advs__container _container">
                    <div class="advs__row">
                        <div class="advs__item">
                            <img src="img/project_5/11.png" alt="hooli">
                        </div>
                        <div class="advs__item">
                            <img src="img/project_5/12.png" alt="lya">
                        </div>
                        <div class="advs__item">
                            <img src="img/project_5/13.png" alt="img">
                        </div>
                        <div class="advs__item">
                            <img src="img/project_5/14.png" alt="stripe">
                        </div>
                        <div class="advs__item">
                            <img src="img/project_5/15.png" alt="aws">
                        </div>
                        <div class="advs__item">
                            <img src="img/project_5/16.png" alt="img">
                        </div>
                    </div>
                </div>
            </div>
            {{-- subscribe --}}
            <section class="page__suscribe subscribe">
                <div class="subscribe__container _container">
                    <div class="subscribe__body">
                        <h4 class="subscribe__title">Subscribe For Latest Newsletter</h4>
                        <form action="#" class="subscribe__form">
                            <input type="email" autocomplete="off" placeholder="Your email" required class="subscribe__input">
                            <button type="submit" class="subscribe__button">Subscribe</button>
                        </form>
                    </div>
                </div>
            </section>
            {{-- get in touch --}}
            <section class="page__getintouch getintouch">
                <div class="getintouch__container _container">
                    <div class="getintouch__header title-block">
                        Get In Touch
                    </div>
                    <div class="getintouch__subheader subtitle-block">
                        Problems trying to resolve the conflict between<br>
                        the two major realms of Classical physics: Newtonian mechanics
                    </div>
                    <div class="getintouch__items">
                        <div class="getintouch__item item-getintouch">
                            <div class="item-getintouch__icon">
                                <img src="img/project_5/17.png" alt="Get In Touch">
                            </div>
                            <div class="item-getintouch__emails">
                                <a href="mailto:georgia.young@example.com" class="item-getintouch__email">georgia.young@example.com</a>
                                <a href="mailto:georgia.young@ple.com" class="item-getintouch__email">georgia.young@ple.com</a>
                            </div>
                            <div class="item-getintouch__label">
                                Get Support
                            </div>
                            <a href="#" class="item-getintouch__button">
                                <p>Submit Request</p>
                            </a>
                        </div>
                        <div class="getintouch__item item-getintouch item-getintouch_active">
                            <div class="item-getintouch__icon">
                                <img src="img/project_5/18.png" alt="Get In Touch">
                            </div>
                            <div class="item-getintouch__emails">
                                <a href="mailto:georgia.young@example.com" class="item-getintouch__email">georgia.young@example.com</a>
                                <a href="mailto:georgia.young@ple.com" class="item-getintouch__email">georgia.young@ple.com</a>
                            </div>
                            <div class="item-getintouch__label">
                                Get Support
                            </div>
                            <a href="#" class="item-getintouch__button">
                                <p>Submit Request</p>
                            </a>
                        </div>
                        <div class="getintouch__item item-getintouch">
                            <div class="item-getintouch__icon">
                                <img src="img/project_5/19.png" alt="Get In Touch">
                            </div>
                            <div class="item-getintouch__emails">
                                <a href="mailto:georgia.young@example.com" class="item-getintouch__email">georgia.young@example.com</a>
                                <a href="mailto:georgia.young@ple.com" class="item-getintouch__email">georgia.young@ple.com</a>
                            </div>
                            <div class="item-getintouch__label">
                                Get Support
                            </div>
                            <a href="#" class="item-getintouch__button">
                                <p>Submit Request</p>
                            </a>
                        </div>
                    </div>
                </div>
            </section>
            {{-- contact --}}
            <div class="page__contact contact">
                <div class="contact__container _container">
                    <div class="contact__row">
                        <div class="contact__column">
                            <div class="contact__header">
                                Consulting Agency For Your Business
                            </div>
                            <div class="contact__subheader">
                                the quick fox jumps over the lazy dog
                            </div>
                        </div>
                        <div class="contact__column">
                            <a href="#" class="contact__btn">Contact Us</a>
                        </div>
                    </div>
                </div>
            </div>
        </main>
        {{-- footer --}}
        <footer class="footer">
            <div class="_container">
                <div class="footer__row">
                    <div class="footer__column footer-column">
                        <div class="footer-column__title">Company Info</div>
                        <a href="#" class="footer-column__item">About Us</a>
                        <a href="#" class="footer-column__item">Carrier</a>
                        <a href="#" class="footer-column__item">We are hiring</a>
                        <a href="#" class="footer-column__item">Blog</a>
                    </div>
                    <div class="footer__column footer-column">
                        <div class="footer-column__title">Legal</div>
                        <a href="#" class="footer-column__item">About Us</a>
                        <a href="#" class="footer-column__item">Carrier</a>
                        <a href="#" class="footer-column__item">We are hiring</a>
                        <a href="#" class="footer-column__item">Blog</a>
                    </div>
                    <div class="footer__column footer-column">
                        <div class="footer-column__title">Features</div>
                        <a href="#" class="footer-column__item">Business Marketing</a>
                        <a href="#" class="footer-column__item">User Analytic</a>
                        <a href="#" class="footer-column__item">Live Chat</a>
                        <a href="#" class="footer-column__item">Unlimited Support</a>
                    </div>
                    <div class="footer__column footer-column">
                        <div class="footer-column__title">Features</div>
                        <a href="#" class="footer-column__item">Business Marketing</a>
                        <a href="#" class="footer-column__item">User Analytic</a>
                        <a href="#" class="footer-column__item">Live Chat</a>
                        <a href="#" class="footer-column__item">Unlimited Support</a>
                    </div>
                    <div class="footer__column footer-column">
                        <div class="footer-column__title">Resources</div>
                        <a href="#" class="footer-column__item">IOS & Android</a>
                        <a href="#" class="footer-column__item">Watch a Demo</a>
                        <a href="#" class="footer-column__item">Customers</a>
                        <a href="#" class="footer-column__item">API</a>
                    </div>
                    <div class="footer__column footer-column">
                        <div class="footer-column__title">Get In Touch</div>
                        <div class="footer-column__line footer-column__line_last">
                            <img src="img/project_5/17.png" alt="phone" width="23px">
                            <a href="tel:+84805550103" class="footer-column__item_last">(480) 555-0103</a>
                        </div>
                        <div class="footer-column__line footer-column__line_last">
                            <img src="img/project_5/20.png" alt="address" width="24px">
                            <a href="#" class="footer-column__item_last">4517 Washington Ave. </a>
                        </div>
                        <div class="footer-column__line footer-column__line_last">
                            <img src="img/project_5/19.png" alt="email" width="23px">
                            <a href="mailto:debra.holt@example.com" class="footer-column__item_last">debra.holt@example.com</a>
                        </div>
                    </div>
                </div>
            </div>
        </footer>
        <div class="footer-end">
            <div class="_container">
                <div class="footer-end__row">
                    <div class="footer-end__text">
                        Made With Love By DigitalCoffeeDesign.com All Right Reserved
                    </div>
                    <div class="footer-end__item">
                        <div class="footer-end__img">
                            <img src="img/logo.png" alt="digitalcoffeedesign.com" width="24">
                        </div>
                        <div class="footer-end__img">
                            <img src="img/project_5/21.png" alt="facebook">
                        </div>
                        <div class="footer-end__img">
                            <img src="img/project_5/22.png" alt="instagram">
                        </div>
                        <div class="footer-end__img">
                            <img src="img/project_5/23.png" alt="tweeter">
                        </div>
                        <div class="footer-end__img">
                            <img src="img/project_5/24.png" alt="youtube">
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <script src="{{ asset('assets/project_5/js/script.js') }}"></script>
</body>
</html>